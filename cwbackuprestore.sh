...
CHOICES=$(whiptail --separate-output --checklist "Choose options" 10 40 3 \
  "1" "Restore one application's Backup" ON \
  "2" "Restore all application's Backup" OFF 3>&1 1>&2 2>&3)
...
if [ -z "$CHOICES" ]; then
  ...
  for CHOICE in $CHOICES; do
    case "$CHOICE" in
    "1")
        read -p "Enter backup date (In YYYY-MM-DD format): " date1
        if [ "z$date1" != "z" ] && date -d "$date1" >/dev/null
        then
            read -p "Enter App's DB name : " dbname
            if [ -d /home/master/applications/$dbname ]; then
	            echo "Restoring : $dbname"
	            /var/cw/scripts/bash/duplicity_restore.sh --src $dbname -r --dst /home/master/applications/$dbname/private_html/ --time $date1
